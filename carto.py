...
total_rows = max([state[1] for state in house_aRangement])
total_cols = max([state[0] for state in house_aRangement])
...
for r in range(total_rows+1):
    ...
    for c in range(total_cols+1):
        inserted = False
        for district in house_aRangement:
            if c == district[0] and r == district[1]:
                district = district[2]
                cells.append({'district':district})
                inserted = True
        if not inserted:
            cells.append({'district':""})
    rows.append(cells)
